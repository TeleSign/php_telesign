...
class RestClient {
  ...
  const REST_ENDPOINT = "https://rest-api-test.telesign.com";
  ...
  protected function execute ($method_name, $resource, $fields = [], $date = null, $nonce = null, $content_type = null, $auth_method = "HMAC-SHA256") {
    $content_is_json = $content_type === "application/json" && in_array($method_name, ["POST", "PUT", "PATCH"]);
  ...
  private function bootstrapRouteMap() {  
    if (method_exists($this, 'loadAutomatedRoutes')) {
      $this->routeMap = $this->loadAutomatedRoutes();
    }
  }

  /**
   * Magic method that captures requests to autogenerated methods.
   */
  public function __call($name, $arguments) {
    if (!isset($this->routeMap[$name])) {
      throw new \BadMethodCallException("Method '{$name}' is not available in " . get_class($this) . " This client may not have generated routes.");
    }
    return $this->executeRoute($this->routeMap[$name], $arguments);
  }

  /**
   * Dynamically processes autogenerated routes.
  */
  private function executeRoute(array $routeConfig, array $arguments) {
    $method = strtoupper($routeConfig['method']);
    $path = $routeConfig['path'];
    $auth_method = isset($routeConfig['auth']) ? $routeConfig['auth'] : "HMAC-SHA256";
    $content_type = isset($routeConfig['contentType']) ? $routeConfig['contentType'] : null;

    $processedFields = array();
    $argIndex = 0;

    $isJsonPost = ($content_type === "application/json" && in_array($method, array("POST", "PUT", "PATCH")));

    if ($isJsonPost && isset($arguments[0]) && is_array($arguments[0])) {
      $processedFields = $arguments[0];
      
      foreach ($routeConfig['parameters'] as $paramName => $spec) {
        $paramIn = isset($spec['in']) ? $spec['in'] : '';
        if ($paramIn === 'path' && isset($processedFields[$paramName])) {
          $path = str_replace("{" . $paramName . "}", urlencode((string)$processedFields[$paramName]), $path);
          unset($processedFields[$paramName]); 
        }
      }
    } else {
      foreach ($routeConfig['parameters'] as $paramName => $spec) {
        ...
        $currentValue = $arguments[$argIndex];
        $paramIn = isset($spec['in']) ? $spec['in'] : '';

        if ($paramIn === 'path') {
          $path = str_replace("{".$paramName."}", urlencode((string)$currentValue), $path);
        } else {
          $processedFields[$paramName] = $currentValue;
        }
    ...
    return $this->execute($method, $path, $processedFields, null, null, $content_type, $auth_method);
  }
}
